...
**「停用宿主内置搜索」这件事整个交还给用户**：面板不再有那个开关，本插件也不再向宿主写任何状态。
理由是真机统计——从 v0.2.0 引入到 v0.9.6，19 次启动里这条后台补关**次次报失败**（`host takeover
re-asserted: ok=False 未能连接宿主管理接口`），而没有一次是真失败：宿主的 `POST /plugin/web_search/stop`
在它重载插件列表时要 8 秒以上才回话，恰好晚于客户端 8.0 秒的等待上限，于是"已经做成的事"被报成错误。
v0.9.7 改成"记意图 + 每 20 秒后台核对"之后，启动不再白等，但留下另一个不彻底：核对成功一次就撒手，
内置要是之后被启回来（用户在插件中心点了一下、或宿主升级重置了偏好），就没有人再管它。

- **删除**：入口 `set_host_search`（14 → 13 条）、启动后的 20 秒核对定时器、配置项
  `[host].takeover_search`、`_host.py` 里的整条写入路（`set_enabled` 及其错误分类与相应文案）。
- **保留**：只读的 `get_host_search` 与面板那张卡——照样显示内置"在跑 / 已停"，`在跑` 时直接告诉你
  去宿主「插件中心」停用，并保留「重新检查」按钮（读状态很便宜，走宿主进程内缓存）。
- 面板文案：中英各删 8 条、改 4 条、新增 1 条（键集 160 → 153，两语言仍一一对应）；
  可见文案总量从 1394 字降到 1275 字（预算 1560）。
- 文档同步：`README.md` 新增"为什么停用内置要用户自己动手"，`docs/quickstart.md` 第 4 问改成
  手动步骤，`docs/plan-v0.2.md` 顶部标出作废项，避免以后有人照着老施工单把写入路加回来。
- 测试 280 → 267 条：删掉 22 条写入路用例，换成 9 条只读与守卫用例（**源码级**钉住"`_host.py` 里没有
  `set_enabled`、没有任何 `POST`"、"插件类上没有 `set_host_search` / 定时器 / `takeover` 配置"、
  启动一次都不碰宿主管理接口、状态读取拿不到答案时降级成中文提示而不是抛错）。
- **并入了远端分支的 anysearch 改动**（远端那两个提交与上面这条删除合在同一个 0.9.8 里发出）：
  ...
  - **面板可以填与测试 anysearch 密钥了**（`set_anysearch_key` / `test_anysearch_key` / `clear_anysearch_key`）：
    ...
    入口在「密钥管理」卡里默认收起的折叠块中。
  - 合并处的取舍：被删的是 `set_host_search` 那一条，anysearch 那三条保留；面板上下文字段随之去掉
    `takeover` / `takeover_error`（`_build_panel_context` 已不再返回它们，而那条断言是精确匹配）。

- **老用户配置钉住旧默认这件事修掉了**。宿主在**首次装包时**把 `config.example.toml` 整份复制成运行时
  配置，之后**不再回写已有的文件**（`config_paths.py:148-153`），所以 0.9.6 时代装的用户配置里留着
  `backend_chain = ["exa", "anysearch", "bing", "baidu"]`，`58f1db5` 改的新默认对他们等于没改——真机表现
  是每次 `search answered: backend=exa attempted=['exa']`，anysearch 一次都没轮到（用户报"没填 key 为什么
  还走 exa"就是这个）。现在 `_chain()` 把**恰好等于那份旧默认**的值当作"没写过"，改用代码默认顺序。
  边界刻意收得很窄：**只认这一份精确列表**，用户手写的任何其它顺序照旧全权生效；想让 exa 打头有一条
  一等公民的路——`[search] backend = "exa"`，它优先于链路顺序、也不受这次纠偏影响（三条新用例分别钉住
  这三件事）。判定只做在读取侧，**不去写用户配置**（宿主的配置写入有"落地了但确认丢了"的前科），改在
  日志里留一条 `backend_chain is the retired default ...` 说明为什么没照配置文件走；配置文件本身不改，
  所以面板上的「现在会用到哪些搜索来源」显示的是生效顺序，而不是文件里那行旧值。

**面板整体重做**。首屏不再是教用户注册密钥的三步教程，而是那件必须做的事——看宿主内置『网络搜索』还在不在跑：

- **首屏只做检查**：两个徽章（内置：在跑 / 本插件：在运行）+「我关好了，查一下」。内置还在跑就
  每次打开面板都回到这一页；查过确认已停用就不再出现；**状态读不到时不算"没关"、也不弹**——把已经
  做成的事报成失败是这个插件的老毛病，不再犯。旁边留一个「先进去看看」，想先配密钥的人不被锁在门外
  （只记本次会话，不写任何配置）。
- **原三步教程搬进「进阶」**，收在 Exa 卡的「怎么拿到一把 Exa 密钥」折叠块里（连"注册时那三个问题
  怎么选"一起）；**池子里有钥匙之后折叠块自动收起**，没配时默认展开。
- **「设置」标签页改名「进阶」**，并把 Exa 与 AnySearch **拆成两张独立卡**（原先挤在一张卡里，而两家
  机制完全不同：一个是轮转的环、一个只有一把表），各带自己的状态、输入框与教程；AnySearch 的注册入口
  `https://www.anysearch.com/about` 一并写进教程和引导文档。
- 新增一张「密钥账号小提醒」：两家都可以在邮件底部点 unsubscribe 退订广告，**退订只是停掉邮件**，
  注册好的账号和密钥照常能用，额度、限速都不受影响。
- 宿主那张卡从「设置」挪到**「状态」**页——它讲的是宿主的事实，不是本插件能改的配置。
- 「现在会用到哪些搜索来源」那张卡上的**「为什么少了某个来源 · 额度怎么算」折叠块删掉**：duckduckgo
  为什么要检测到代理才启用，引导文档的代理一节已经写了；额度算法留在 Exa 卡的「一把不够用怎么办」里。
  同一件事不在两张卡上各放一份。
- **撤掉"引导走到哪一步"那套状态**：入口 `set_onboarding`、`show_guide` 与配置项 `[ui].onboarding_stage`
  一起删除（面板入口 16 → 14 条）。首屏从此只由客观状态决定，不再依赖"用户点过没有"这种隐藏进度。
  老用户配置里那行 `onboarding_stage` 变成没人读的死字段，与 `takeover_search` 同样处理：不写配置去清它。
  `[ui].first_run_notice_sent` 保留，仍然只管"首启聊天提示只发一次"。
- 顺手修掉首启聊天提示里的一句**假话**：它还在说"面板里还能一键停用系统自带的『网络搜索』"，而那个能力
  正是本版撤掉的。
- 文档同步：`docs/quickstart.md` 改成"第一节就是关内置"，并新增「想开回来怎么办」「账号老发广告邮件
  怎么办」两问；README 里"一键停用宿主内置搜索""三步引导 + 先体验按钮"等说法一并改正（README 会打进
  分发包）。施工单 `docs/plan-v0.2.md` 顶部补第二条作废说明，免得有人照着老契约把 `set_onboarding` 加回来。
- 注册教程的中文措辞统一成"**登录选邮箱登录**"，不再在中文文案里塞英文按钮名（面板与引导文档同改）。

**合完重新量的数**：面板入口 13 → 14 条（随 anysearch 多出 3 条，又随引导机制撤掉 2 条）；中英键集 170 条、
一一对应；可见文案 1253/1560 字（最长单条 58/60，占字最多的是自检卡 248 字）；测试 285 → 281 条（删掉 4 条
引导进度用例）。`ui/panel.tsx` 这次
**跑了类型检查且退出 0**——按 `README.md` 里那份 `tsc` 配方跑的，v0.9.7 里"没跑过类型检查"那句作废。

**升级提示**：已经装过 0.9.6 / 0.9.7 的机器，配置里那个 `[host] takeover_search = true` 会变成没人读的
死字段（留着无害，想清掉直接删那一行）。**内置搜索会不会继续跑，取决于插件中心里那份开关的状态**：
之前被本插件停过的，宿主的 `plugin_runtime_overrides.json` 里已经是"停用 + 不开机自启"，不会自己回来；
如果哪天发现它又在了，去插件中心停一下即可。

### 这一版还没在真机上验过的事

1. **首位分叉只跑过离线用例**：真机上要确认的是保存一把 Exa 密钥之后，面板「现在会用到哪些搜索来源」里
   exa 的徽章确实排到第一个，且「最近一次搜索」显示这一轮由 exa 答的。
2. **anysearch 那行面板从没渲染过**：新卡里的两个 StatCard、折叠块里的输入框与三个按钮（保存/测这把/移除）
   都要起宿主装包看一遍；尤其要确认「移除」按钮只在已有密钥时才出现，以及折叠块展开后没把卡片撑得难读。
3. **「宿主内置『网络搜索』」那张卡改成只读之后没渲染过**：要看的是它只剩状态徽章 +「重新检查」按钮、
   不再有开关，且"在跑"时把用户指去宿主的插件中心。
