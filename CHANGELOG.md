...
v0.9.6 之后累计的改动一次发出去，编号取 **1.0.0**：免密钥可用的定位、链路默认顺序、面板首屏与
密钥配置方式都已定型，没有留给"下一版再做"的东西。
...
- **并入了远端分支的 anysearch 改动**（远端那两个提交与上面这条删除合在同一个 1.0.0 里发出）：
...
- 按钮「再加一把」改成「**加一把**」（英文同步 `Add another key` → `Add a key`）：第一把还没填的时候，
  "再"字让人以为这按钮是给第二把用的。面板、教程折叠块、后端那句"密钥没验通过"的提示与引导文档一起跟上；
  更新记录里的历史条目按原样保留。

**合完重新量的数**：面板入口 13 → 14 条（随 anysearch 多出 3 条，又随引导机制撤掉 2 条）；中英键集 170 条、
一一对应；可见文案 1252/1560 字（最长单条 58/60，占字最多的是自检卡 248 字）；测试 285 → 281 条（删掉 4 条
